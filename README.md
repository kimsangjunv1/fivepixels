...
`@fivepixels-js/react/report` subpath는 `FivePixels`, `ReportProvider`, persistence 타입만 export하며 `motion`은 포함하지 않습니다.
...
```tsx
import { FivePixels, formatFeedbackAsGitHubIssueBody, type ReportFeedback } from "@fivepixels-js/react";
...
import { formatFeedbackAsGitHubIssueBody } from "@fivepixels-js/react";
