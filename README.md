...
| 이름 | 설명 |
| --- | --- |
| `projectId` | 프로젝트 식별자. 생략 시 `"my-app"`. |
| `environment` | 환경별 피드백 분리 (`local`, `dev`, `stage`, `production` 등). |
| `appVersion` | 피드백 생성 시 기록되는 앱/서비스 버전. |
| `appearance` | UI 테마. `"light"` \| `"dark"` \| `"system"` (기본값: `"system"`). |
| `storage` | `"local"`(기본) 또는 `ReportStorageAdapter`. |
| `storageAdapter` | 커스텀 `ReportStorageAdapter`. API 연동 시 권장. |
| `fields` | 피드백 작성 폼 필드 배열. |
| `authors` | 답변·검수 시 선택할 작성자 목록. |
| `identify` | 최초 피드백 작성자 `{ id, name }`. |
| `pathname` | 피드백 저장·조회 기준 경로. 생략 시 `window.location.pathname`. |
| `showFeedbackList` | view 모드 우측 목록 패널 표시 여부 (기본값: `true`). |
| `visibleShortcutKeys` | 버튼 옆 단축키 힌트 표시 (기본값: `false`). |
| `devOnly` | `true`면 production에서 UI 미렌더링. |
| `enabled` | `false`면 환경과 관계없이 UI 미렌더링 (기본값: `true`). |
| `onCreate` | 피드백 생성 callback. |
| `onUpdate` | 피드백 수정 callback. |
| `onDelete` | 피드백 삭제 callback. |
| `onReply` | 피드백 답변 추가 callback. |
| `onEvent` | 모든 피드백 이벤트 통합 callback (`ReportEvent`). |
...
- `onCreate`, `onUpdate`, `onDelete`, `onReply`: 이벤트별 callback
...
- `onFeedbackCreate` / `onFeedbackUpdate` / `onFeedbackDelete` / `onFeedbackReply`는 각각 `onCreate` / `onUpdate` / `onDelete` / `onReply`로 변경되었습니다.
