...
- `FivePixels`는 피드백을 받을 화면에 1회만 렌더링합니다.
...
- Next.js 등 SSR 환경에서는 `FivePixels`를 **클라이언트에서만** 렌더링하세요. (`"use client"` + dynamic import 등)

## Full Example

아래는 `<FivePixels />`의 주요 prop을 **한 번에** 연결한 예시입니다. 로컬만 쓸 때는 `onList`/`onCreate`/`onUpdate`/`onDelete`를 생략하고, GitHub·side effect가 필요 없으면 `github`/`onEvent`/`onReply`를 빼면 됩니다.
...
`FivePixels` UI는 호스트 앱 React 트리와 분리되어 **Shadow Root**(`#fivepixels-root`)에 렌더링됩니다.
...
- `appearance="light" | "dark" | "system"`은 Shadow Root 내부 `data-fivepixels-theme`으로 반영됩니다.
...
```tsx
import { motion, AnimatedPresence } from "fivepixels";
```

`fivepixels/report` subpath는 `FivePixels`, `ReportProvider`, persistence 타입만 export하며 `motion`은 포함하지 않습니다.
...
- localStorage 기본 키는 `fivepixels:reports:v1:{projectId}` 또는 `fivepixels:reports:v1:{projectId}:{environment}` 형태입니다.
...
```ts
import type { ReportEvent } from "fivepixels";
...
```tsx
import { FivePixels, formatFeedbackAsGitHubIssueBody, type ReportFeedback } from "fivepixels";
...
import { formatFeedbackAsGitHubIssueBody } from "fivepixels";
...
- `github.onCreate`는 persistence handler `onCreate`와 **이름만 같을 뿐 별개**입니다. 피드백 저장은 fivepixels이 먼저 수행하고, 성공 후 `onCreate` → `onUpdate({ status, integrations, replies })` 순으로 반영합니다.
...
- 기본 `FivePixels` 컴포넌트는 view 모드 마커 UI에서 **답변 작성·검수(denied / confirm / checkout)** 를 지원합니다.
