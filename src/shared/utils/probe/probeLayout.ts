import type { PickProbeLayoutMode } from "@/shared/types/report-ui.js";

export const PROBE_GRID_TRACK_MIN = 1;
export const PROBE_GRID_TRACK_MAX = 12;

export const FLEX_JUSTIFY_VALUES = ["flex-start", "center", "flex-end", "space-between"] as const;
export const FLEX_ALIGN_VALUES = ["flex-start", "center", "flex-end"] as const;

export type FlexJustifyValue = (typeof FLEX_JUSTIFY_VALUES)[number];
export type FlexAlignValue = (typeof FLEX_ALIGN_VALUES)[number];

export function getPickProbeLayoutMode(element: HTMLElement): PickProbeLayoutMode {
...
export function isColumnFlexDirection(direction: string) {
    return direction.includes("column");
}

export function isFlexReversed(direction: string) {
    return direction.includes("reverse");
}

export function getFlexAxis(direction: string): "row" | "column" {
    return isColumnFlexDirection(direction) ? "column" : "row";
}

export function buildFlexDirection(axis: "row" | "column", reversed: boolean) {
    if (axis === "row") {
...
export function toggleFlexReverse(direction: string) {
    const axis = getFlexAxis(direction);
    return buildFlexDirection(axis, !isFlexReversed(direction));
}

export function parseGridTrackCount(template: string) {
...
export function clampGridTrackCount(value: number) {
...
export function formatGridTrackCount(count: number) {
    return `repeat(${clampGridTrackCount(count)}, minmax(0, 1fr))`;
}

export function parseProbeGap(value: string) {
...
export function captureProbeGap(style: CSSStyleDeclaration) {
    const gap = style.gap || style.rowGap || style.columnGap;

    return parseProbeGap(gap);
}

export function stepProbeGap(value: string, delta: number) {
...
export function stepGridTrackCount(value: string, delta: number) {
