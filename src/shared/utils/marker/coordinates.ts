...
import type { DraftReport, Marker, MarkerClampBounds, MarkerClampEdge, MarkerOverflowHint, TargetSnapshot } from "@/shared/types/report-ui.js";
...
function applyScrollContainerClamp(left: number, top: number, element: HTMLElement | null): MarkerPosition {
    ...
    const clampBounds: MarkerClampBounds = {
...
function getAnchorMarkerPosition(report: ReportFeedback) {
    ...
    return applyScrollContainerClamp(
        rect.left + rect.width * anchor.x - getMarkerDotSize() / 2,
        rect.top + rect.height * anchor.y - getMarkerDotSize() / 2,
        anchorElement,
    );
}
...
export function getMarkerFromReport(report: ReportFeedback, currentScrollY: number): Marker {
    ...
    if (viewTrigger) {
        ...
        const markerPosition = applyScrollContainerClamp(
            rect.left + rect.width / 2 - getMarkerDotSize() / 2,
            rect.top + rect.height / 2 - getMarkerDotSize() / 2,
            viewTrigger.element,
        );
...
export function aggregateViewTriggerMarkers(markers: Marker[]) {
    const aggregated: Marker[] = [];
...
export function getDraftMarkerPosition(
    ...
    if (selectedTarget) {
        ...
        return applyScrollContainerClamp(
            selectedTarget.rect.left + selectedTarget.rect.width * draft.elementXRatio - getMarkerDotSize() / 2,
            selectedTarget.rect.top + selectedTarget.rect.height * draft.elementYRatio - getMarkerDotSize() / 2,
            targetElement,
        );
    }
...
function serializeClampBounds(bounds: MarkerClampBounds) {
    return `${bounds.left}:${bounds.top}:${bounds.right}:${bounds.bottom}`;
}

function getOverflowHintPosition(edge: MarkerClampEdge, bounds: MarkerClampBounds) {
...
export function resolveMarkerOverflowHints(markers: Marker[]): MarkerOverflowHint[] {
    const grouped = new Map<string, MarkerOverflowHint>();
