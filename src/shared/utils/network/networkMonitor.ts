import type { ApiFlowEntry, ApiFlowFailureKind, NetworkMonitorSnapshot } from "@/shared/types/networkMonitor.js";
import { isRscNetworkRequest, parseApiFlowUrl } from "./formatApiFlowEntry.js";

const MAX_ENTRIES = 100;
const MAX_BODY_LENGTH = 32_768;
const FAILURE_ALERT_TTL_MS = 30_000;
...
let entries: ApiFlowEntry[] = [];
let dismissedFailureAlertId: string | null = null;
const listeners = new Set<() => void>();
let failureAlertTimeoutId: number | null = null;

const EMPTY_SNAPSHOT: NetworkMonitorSnapshot = { entries: [], activeFailureAlert: null };
let cachedSnapshot: NetworkMonitorSnapshot = EMPTY_SNAPSHOT;
...
function resolveActiveFailureAlert(): ApiFlowEntry | null {
...
function scheduleFailureAlertExpiry(entry: ApiFlowEntry) {
...
function serializeRequestBody(body: BodyInit | Document | null | undefined): string | null {
    if (body == null) {
        return null;
    }
...
function pushEntry(entry: ApiFlowEntry) {
...
function buildEntryBase(method: string, url: string) {
...
function finalizeEntry(
    base: ReturnType<typeof buildEntryBase>,
    params: {
        startedAt: number;
        status: number | null;
        ok: boolean;
...
async function readRequestBodyFromRequest(request: Request): Promise<string | null> {
    try {
...
async function recordFetch(input: RequestInfo | URL, init?: RequestInit) {
    if (!originalFetch) {
        return undefined;
    }
...
function patchFetch() {
    ...
    window.fetch = ((input: RequestInfo | URL, init?: RequestInit) => recordFetch(input, init)) as typeof fetch;
}
...
function patchXhr() {
    ...
    XMLHttpRequest.prototype.send = function send(body?: Document | XMLHttpRequestBodyInit | null) {
        ...
        const finalizeFromXhr = (failureKind: ApiFlowFailureKind | null, errorMessage: string | null) => {
            if (recorded) {
                return;
            }
...
export function subscribeNetworkMonitor(listener: () => void) {
    listeners.add(listener);
    return () => listeners.delete(listener);
}

export function getNetworkMonitorSnapshot(): NetworkMonitorSnapshot {
    return cachedSnapshot;
}

export function getServerNetworkMonitorSnapshot(): NetworkMonitorSnapshot {
    return EMPTY_SNAPSHOT;
}

export function dismissNetworkFailureAlert(entryId: string) {
...
export function getApiFlowEntryById(entryId: string): ApiFlowEntry | undefined {
