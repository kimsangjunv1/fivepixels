...
} from "@/shared/utils/overlay/minimizedDockLayout.js";
...
export function useOverlayMinimizedDock({ windowId, enabled, isMinimized, onMinimizedChange }: UseOverlayMinimizedDockOptions) {
    ...
    const dockPosition = useMemo(() => {
        ...
        return resolveMinimizedDockPosition(
            dockIndex,
            dockCount,
            viewportWidth,
            viewportHeight,
    ...
    const minimizeToDock = useCallback(
        (from: DockMorphRect) => {
            ...
            const viewportWidth = typeof window === "undefined" ? 1280 : window.innerWidth;
            const viewportHeight = typeof window === "undefined" ? 720 : window.innerHeight;
            const to = resolveMinimizedDockPosition(
                nextIndex,
                nextOrder.length,
                viewportWidth,
                viewportHeight,
    ...
    const layoutTransition =
        dockMorph ? MINIMIZE_MORPH_TRANSITION : isMinimized && !isSelfDragging ? MINIMIZED_DOCK_SLIDE_TRANSITION : undefined;
