...
import { getNearestPanelCorner, getStoredPanelCorner, type PanelCorner } from "@/shared/hooks/usePanelDock.js";
...
export function resolveNotificationExpandDirection(corner: PanelCorner): NotificationStackExpandDirection {
    return corner.startsWith("bottom") ? "up" : "down";
}

function findHostPanel(stackNode: HTMLElement | null): HTMLElement | null {
    if (!stackNode) {
        return null;
    }
    ...
    if (root instanceof ShadowRoot || root instanceof Document) {
        const panel = root.querySelector<HTMLElement>('[data-fp-chrome="panel"]:not([data-embedded])');
    ...
    return document.querySelector<HTMLElement>('[data-fp-chrome="panel"]:not([data-embedded])');
}

export function resolveCornerFromPanelRect(panel: HTMLElement): PanelCorner {
    const rect = panel.getBoundingClientRect();
    return getNearestPanelCorner(rect.left + rect.width / 2, rect.top + rect.height / 2);
}

export function buildAnchorFromPanel(panel: HTMLElement, corner: PanelCorner): Omit<NotificationStackAnchor, "ready"> {
    const rect = panel.getBoundingClientRect();
    ...
    const horizontal: CSSProperties = corner.endsWith("right") ? { right: Math.max(0, window.innerWidth - rect.right), left: "auto" } : { left: Math.max(0, rect.left), right: "auto" };
...
function fallbackAnchor(corner: PanelCorner, ready: boolean): NotificationStackAnchor {
    const expandDirection = resolveNotificationExpandDirection(corner);
    const horizontal: CSSProperties = corner.endsWith("right") ? { right: 16, left: "auto" } : { left: 16, right: "auto" };
...
function anchorsEqual(left: NotificationStackAnchor, right: NotificationStackAnchor) {
    return (
 ...
 */
export function useNotificationStackAnchor(enabled: boolean, stackRef: RefObject<HTMLElement | null>): NotificationStackAnchor {
    const [anchor, setAnchor] = useState<NotificationStackAnchor>(() => fallbackAnchor(getStoredPanelCorner(), false));
    ...
    useLayoutEffect(() => {
        ...
        const commitAnchor = (next: NotificationStackAnchor) => {
        ...
        let startDragLoop: () => void;
        let attachPanelObservers: (panel: HTMLElement) => void;
        ...
        attachPanelObservers = (panel: HTMLElement) => {
        ...
        window.addEventListener("fivepixels:panel-placement", handlePlacement as EventListener);
        ...
        return () => {
            ...
            window.removeEventListener("fivepixels:panel-placement", handlePlacement as EventListener);
        };
    }, [enabled, stackRef]);
