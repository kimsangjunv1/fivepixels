            ...
            {/* {persistenceStatus.mode === "conflict" ? (
                <details className="group relative py-[4px] normal-case">
                    ...
                    <div className="absolute right-0 bottom-full z-[30] mb-[6px] w-[300px] rounded-[10px] border border-[#f59e0b] bg-[var(--adaptive-black50)] p-[12px] text-left normal-case shadow-[0_12px_32px_rgba(15,23,42,0.24)]">
                        <p className="text-[12px] font-[700] text-[var(--adaptive-black900)]">{messages.panel.persistenceConflictTitle}</p>
                        <p className="mt-[4px] text-[11px] leading-[1.5] text-[var(--adaptive-black600)]">{messages.panel.persistenceConflictDescription}</p>

                        <p className="mt-[10px] text-[11px] font-[700] text-[var(--adaptive-black700)]">{messages.panel.persistenceRequiredForApi}</p>
                        <ul className="mt-[4px] space-y-[2px] font-mono text-[11px] text-[var(--adaptive-black700)]">
                            {(["onList", "onCreate", "onUpdate"] as const).map((handlerName) => {
                                ...
                                return (
                                    <li
                                        key={handlerName}
                                        className={missing ? "text-[#dc2626]" : "text-[#15803d]"}
                                    >
                                        {missing ? "✕" : "✓"} {handlerName}
                                    </li>
                                );
                        ...
                        <p className="mt-[10px] text-[11px] font-[700] text-[var(--adaptive-black700)]">{messages.panel.persistenceProvidedButIgnored}</p>
                        <p className="mt-[4px] break-words font-mono text-[11px] text-[var(--adaptive-black600)]">{persistenceStatus.ignoredHandlers.join(", ")}</p>
