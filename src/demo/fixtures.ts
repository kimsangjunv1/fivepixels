...
function buildReportFromSample(
    sample: (typeof STATUS_SAMPLES)[number],
    index: number,
    idOverride?: string,
): ReportFeedback {
...
export const DEMO_FEATURED_REPORTS: ReportFeedback[] = STATUS_SAMPLES.map((sample, index) => buildReportFromSample(sample, index));

type ActivityVolume = {
    replyStatus?: ReportReplyStatus;
    reportStatus: ReportFeedback["status"];
    today: number;
    yesterday: number;
};

/** 패널 Today/Yesterday·스파크라인용 0~50 분포 */
const ACTIVITY_VOLUME: ActivityVolume[] = [
...
function pickHour(index: number, daysAgo: number, seed: number): number {
...
function buildActivityReports(featured: ReportFeedback[]): ReportFeedback[] {
    const generated: ReportFeedback[] = [];
    ...
    ACTIVITY_VOLUME.forEach((volume, volumeIndex) => {
        ...
        const days: Array<{ daysAgo: number; count: number }> = [
...
export const DEMO_REPORTS: ReportFeedback[] = buildActivityReports(DEMO_FEATURED_REPORTS);
...
export const DEMO_MEMO_DRAFT: DraftReport = {
    ...DEMO_DRAFT,
    category: "memo",
    fieldValues: {},
    cases: [
