...
const STORAGE_KEY = "stitchable:reports:v1";
...
function writeAll(items: ReportFeedback[]) {
    if (typeof window === "undefined") {
        return;
    }

    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(items.map(normalizeReport)));
}

export function createLocalStorageReportAdapter(): ReportStorageAdapter {
    return {
        async list({ pathname }) {
            return readAll()
