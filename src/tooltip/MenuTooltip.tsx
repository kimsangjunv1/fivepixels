 ...
 */
export const MenuTooltipSurface = forwardRef<HTMLDivElement, MenuTooltipSurfaceProps>(function MenuTooltipSurface({ children, className = "", style, positioning = "absolute", ...rest }, ref) {
    ...
    return (
        <div
            ...
            style={style}
            className={`${positionClass} min-w-[160px] ${MENU_TOOLTIP_SURFACE_BASE} ${className}`}
            {...rest}
        >
            {children}
        </div>
...
export function MenuTooltipItem({ children, onClick, active = false, disabled = false, danger = false, icon, className = "", onPointerDown, "aria-pressed": ariaPressed }: MenuTooltipItemProps) {
    return (
        <button
            type="button"
            role="menuitem"
            ...
            onClick={onClick}
            className={`${MENU_TOOLTIP_ITEM_CLASS} ${active ? MENU_TOOLTIP_ITEM_ACTIVE_CLASS : ""} ${danger ? MENU_TOOLTIP_ITEM_DANGER_CLASS : ""} ${className}`}
        >
            {icon}
            {children}
        </button>
...
export function MenuTooltipDivider() {
    return (
        <div
            role="separator"
            aria-hidden="true"
            className={MENU_TOOLTIP_DIVIDER_CLASS}
        />
    );
}
