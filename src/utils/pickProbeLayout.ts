...
export function getPickProbePanelLayout(anchorRect: DOMRect, panelWidth: number, panelHeight: number): PickProbePanelLayout {
    ...
    let placement: PickProbePanelPlacement = "below";
    let top = belowTop;
    ...
    const left = Math.min(
        ...
        Math.max(PANEL_MARGIN, viewportWidth - PANEL_MARGIN - panelWidth),
    );

    return { top, left, placement };
}

export function getPickProbeCompareChipLayout(anchorRect: DOMRect, chipWidth: number, chipHeight: number) {
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
    const margin = 8;
    const gap = 6;
