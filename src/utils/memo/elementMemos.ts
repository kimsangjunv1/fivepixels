...
const DEFAULT_POSITION: ElementMemoPosition = {
    elementXRatio: 0.5,
    elementYRatio: 0.5,
};

function normalizePosition(value: unknown): ElementMemoPosition {
...
function readMemoMap(projectId: string, pathname: string): ElementMemoMap {
...
function writeMemoMap(projectId: string, pathname: string, memos: ElementMemoMap) {
    if (typeof window === "undefined") {
        return;
    }
...
export function readElementMemos(projectId: string, pathname: string) {
    return readMemoMap(projectId, pathname);
}

export function saveElementMemo(
    projectId: string,
    pathname: string,
    elementKey: string,
    text: string,
    position?: ElementMemoPosition,
) {
...
export function deleteElementMemo(projectId: string, pathname: string, elementKey: string) {
    const current = readMemoMap(projectId, pathname);
    delete current[elementKey];
    writeMemoMap(projectId, pathname, current);
    return current;
