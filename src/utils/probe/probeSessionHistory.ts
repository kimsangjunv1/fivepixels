...
function applyStyleEntry(elementKey: string, entry: SavedProbeEntry) {
...
function withoutDeletion(deletions: SavedProbeDeletion[], id: string) {
    return deletions.filter((entry) => entry.id !== id);
}

function withDeletion(deletions: SavedProbeDeletion[], deletion: SavedProbeDeletion) {
...
export function applyProbeSessionActionForward(action: ProbeSessionAction, maps: ProbeSessionMaps): ProbeSessionMaps {
    switch (action.kind) {
        case "style-apply": {
...
export function applyProbeSessionActionBackward(action: ProbeSessionAction, maps: ProbeSessionMaps): ProbeSessionMaps {
    switch (action.kind) {
        case "style-apply": {
            const edits = { ...maps.edits };
