...
import { usesReplyAppendHistory, usesReplyPaginationMode } from "../../shared/constants/replyHistory.js";
import { appendReplyUnique, getEmbeddedRepliesForHistory, getPaginationWindow, getTotalReplyCount, paginateSortedReplies, prependReplies, sortRepliesChronologically, } from "../../shared/utils/feedback/replyHistory.js";
import { listReplies as listRepliesApi } from "../../shared/hooks/report.api.js";
...
async function fetchReplyPage(adapter, report, usesLazyReplies, pageSize, cursor, caseId) {
    if (usesLazyReplies) {
...
function buildInitialPaginationState(report, pageResult) {
...
function buildInitialAppendState(report, pageResult) {
    const totalCount = pageResult.totalCount ?? getTotalReplyCount(report);
    return {
        items: pageResult.items,
        hasMoreOlder: pageResult.hasMore,
...
function flattenPageCache(pageCache) {
    const pages = Object.keys(pageCache)
        .map((key) => Number(key))
        .sort((left, right) => left - right);
    return sortRepliesChronologically(pages.flatMap((pageIndex) => pageCache[pageIndex] ?? []));
}
export function createReplyHistoryActions({ adapter, usesLazyReplies, getReportById, replyHistoryByReportId, setReplyHistoryByReportId, }) {
    const initReplyHistory = useCallback(async (report, config, caseId) => {
    ...
    const loadOlderReplies = useCallback(async (reportId, config, caseId) => {
    ...
    const goToOlderPaginationPage = useCallback(async (reportId, config) => {
        const state = replyHistoryByReportId[reportId];
    ...
    const goToNewerPaginationPage = useCallback((reportId, config) => {
        const state = replyHistoryByReportId[reportId];
    ...
    const appendReplyToHistory = useCallback((reportId, reply) => {
        setReplyHistoryByReportId((current) => {
    ...
    const resetReplyHistory = useCallback((reportId) => {
        if (!reportId) {
            setReplyHistoryByReportId({});
            return;
        }
...
//# sourceMappingURL=replyHistoryActions.js.map