import { type Dispatch, type MouseEvent, type MutableRefObject, type SetStateAction } from "react";
import type { ReportMessages } from "../../../shared/i18n/types.js";
import type { CreateReportFeedbackPayload, ReportAuthor, ReportFeedback, ReportField, ReportIdentify } from "../../../shared/types/report.js";
import type { ElementMention } from "../../../shared/types/mention.js";
import type { DraftReport, HoverPointer, ReportMode, TargetSnapshot } from "../../../shared/types/report-ui.js";
import type { FeedbackCategory } from "../../../shared/constants/feedbackCategory.js";
import type { SessionActor } from "../../../shared/utils/report/reportTeam.js";
import type { ApiFlowEntry } from "../../../shared/types/networkMonitor.js";
export type UseReportDraftSessionParams = {
    mode: ReportMode;
    setMode: Dispatch<SetStateAction<ReportMode>>;
    fields: ReportField[];
    messages: ReportMessages;
    currentPathname: string;
    environment?: string;
    appVersion?: string;
    sessionActor: SessionActor | null;
    authorSelectionLocked: boolean;
    activeIdentify: ReportIdentify | undefined;
    authorizedAuthors: ReportAuthor[];
    selfName?: string;
    setErrorMessage: Dispatch<SetStateAction<string>>;
    hoveredElementRef: MutableRefObject<HTMLElement | null>;
    selectedElementRef: MutableRefObject<HTMLElement | null>;
    overlayRef: MutableRefObject<HTMLDivElement | null>;
    overlayHoverLeaveTimeoutRef: MutableRefObject<number | null>;
};
export declare function useReportDraftSession({ mode, setMode, fields, messages, currentPathname, environment, appVersion, sessionActor, authorSelectionLocked, activeIdentify, authorizedAuthors, selfName, setErrorMessage, hoveredElementRef, selectedElementRef, overlayRef, overlayHoverLeaveTimeoutRef, }: UseReportDraftSessionParams): {
    showTargetPreview: boolean;
    setShowTargetPreview: Dispatch<SetStateAction<boolean>>;
    selectableTargets: TargetSnapshot[];
    draft: DraftReport | null;
    setDraft: Dispatch<SetStateAction<DraftReport | null>>;
    hoveredTarget: TargetSnapshot | null;
    setHoveredTarget: Dispatch<SetStateAction<TargetSnapshot | null>>;
    hoverPointer: HoverPointer | null;
    setHoverPointer: Dispatch<SetStateAction<HoverPointer | null>>;
    selectedTarget: TargetSnapshot | null;
    setSelectedTarget: Dispatch<SetStateAction<TargetSnapshot | null>>;
    draftStep: "category" | "content";
    setDraftStep: Dispatch<SetStateAction<"category" | "content">>;
    draftElementRef: MutableRefObject<HTMLElement | null>;
    draftAuthorName: string;
    setDraftAuthorName: Dispatch<SetStateAction<string>>;
    setDraftAuthorNameSafe: (name: string) => void;
    pickProbeOpen: boolean;
    pickProbeSupportsTextFields: boolean;
    pickProbeLayoutMode: import("../../../shared/types/report-ui.js").PickProbeLayoutMode;
    pickProbeValues: import("../../../shared/types/report-ui.js").PickProbeValues | null;
    pickProbeCompareMode: import("../../../shared/types/report-ui.js").PickProbeCompareMode;
    pickProbeHasEdits: boolean;
    pickTargetContextMenu: import("../../../shared/types/report-ui.js").ContextMenuTooltipState | null;
    contextMenuElementKey: string | null;
    savedProbeEdits: Record<string, import("../../../shared/types/report-ui.js").SavedProbeEntry>;
    savedProbeDeletions: import("../../../shared/types/report-ui.js").SavedProbeDeletion[];
    hasProbeSessionChanges: boolean;
    canUndoProbeSession: boolean;
    canRedoProbeSession: boolean;
    undoProbeSessionAction: () => void;
    redoProbeSessionAction: () => void;
    savedProbeCompareMode: import("../../../shared/types/report-ui.js").PickProbeCompareMode;
    closePickProbe: () => void;
    closeContextMenuTooltip: () => void;
    handlePickTargetEdit: () => void;
    handlePickTargetDelete: () => void;
    handlePickTargetRevert: () => void;
    handlePickTargetMemo: () => void;
    commitPickProbeEdits: () => void;
    revertSavedProbeEdit: (elementKey: string) => void;
    revertAllSavedProbeEdits: () => void;
    setSavedProbeCompareMode: (compareMode: import("../../../shared/types/report-ui.js").PickProbeCompareMode) => void;
    setPickProbeCompareMode: (compareMode: import("../../../shared/types/report-ui.js").PickProbeCompareMode) => void;
    updatePickProbeValue: (key: import("../../../shared/types/report-ui.js").PickProbeFieldKey, value: string) => void;
    resetPickProbeValues: () => void;
    resetPickProbeState: () => void;
    appendSavedProbeSummaryAsNewDraftCase: () => void;
    appendApiFlowEntryToDraftCase: (entry: ApiFlowEntry) => void;
    clearOverlayHoverLeaveTimeout: () => void;
    toggleTargetPreview: () => void;
    handleOverlayMove: (event: MouseEvent<HTMLDivElement>) => void;
    handleOverlayContextMenu: (event: MouseEvent<HTMLDivElement>) => void;
    handleOverlayClick: (event: MouseEvent<HTMLDivElement>) => void;
    cancelDraft: () => void;
    beginDraftEdit: (report: ReportFeedback) => boolean;
    updateDraftCase: (caseId: string, text: string, mentions?: ElementMention[], userMentions?: import("../../../shared/types/mention.js").UserMention[]) => void;
    updateDraftCategory: (category: FeedbackCategory | null) => void;
    addDraftCase: () => void;
    removeDraftCase: (caseId: string) => void;
    updateDraftField: (key: string, nextValue: string | boolean) => void;
    buildCreatePayloadFromDraft: () => CreateReportFeedbackPayload | null;
    finalizeDraftCreate: () => void;
};
//# sourceMappingURL=useReportDraftSession.d.ts.map