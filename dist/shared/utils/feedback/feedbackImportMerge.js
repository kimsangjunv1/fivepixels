import { sortRepliesChronologically } from "../../../shared/utils/feedback/replyHistory.js";
import { mergeRepliesIntoReport } from "../../../shared/utils/report/reportSummary.js";
function pickNewerReply(existing, incoming) {
...
export function mergeReplyCollections(existing, incoming) {
    const byId = new Map();
...
export function mergeFeedbackItem(existing, incoming) {
 ...
 */
export function mergeFeedbackCollections(existing, incoming) {
    ...
    for (const item of incoming) {
        ...
        merged.push(mergeRepliesIntoReport(item, sortRepliesChronologically(item.replies ?? [])));
        inserted += 1;
    }
    return {
        items: merged,
...
//# sourceMappingURL=feedbackImportMerge.js.map