...
export function resolveMinimizedDockIndexFromPointer(centerX, count, viewportWidth, itemWidth = MARKER_MINIMIZED_WINDOW_WIDTH, gap = MARKER_MINIMIZED_DOCK_GAP) {
    const safeCount = Math.max(1, count);
    if (safeCount <= 1) {
        return 0;
    }
