...
export function formatFeedbackAsGitHubIssueBody(feedback, fields = []) {
    const tags = formatCheckboxTags(feedback, fields);
    const threadRows = feedback.replies
        .map((reply) => `| ${reply.created_at} | ${escapeMarkdownTableCell(reply.author_name ?? "-")} | ${reply.status} | ${escapeMarkdownTableCell(reply.message)} |`)
        .join("\n");
    return [
        "## Summary",
        feedback.message,
