...
function applyProbeTextContent(element, value) {
    if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
        element.value = value;
        return;
    }
...
function removeProbeFieldInlineStyle(element, key) {
    const propertyMap = {
        padding: "padding",
        margin: "margin",
        textColor: "color",
...
function applyProbeFieldValue(element, key, value, layoutMode) {
    switch (key) {
        case "padding":
...
export function applyPickProbeValueDiff(element, baseline, current, mode = "after") {
    const supportsTextFields = shouldInspectFontStyle(element);
    const layoutMode = getPickProbeLayoutMode(element);
...
export function applyPickProbeValues(element, values, baseline) {
