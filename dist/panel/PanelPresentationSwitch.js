...
export function PanelPresentationSwitch() {
    ...
    return (_jsx(DropdownMenu, { open: open, onClose: () => setOpen(false), trigger: _jsx(HoverTooltip, { label: messages.panel.presentationSwitchAriaLabel, children: _jsxs("button", { type: "button", "aria-haspopup": "menu", "aria-expanded": open, "aria-label": messages.panel.presentationSwitchAriaLabel, onPointerDown: (event) => {
            ...
            return (_jsx(DropdownMenuItem, { active: viewer.id === activeViewer?.id, disabled: disabled, onClick: () => {
                    if (disabled) {
                        return;
                    }
