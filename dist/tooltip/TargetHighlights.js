...
function ContextMenuFocusBox({ target }) {
...
export function TargetHighlights({ hoveredTarget, selectedTarget = null, contextMenuTarget = null, showHoverInspect = false, showSelectionHighlight = false, showPickProbeCompare = false, showActiveMarkerInspect = false, previewTargets = [], markerPreviewTargets = [], activeMarkerTarget, mentionHighlightTarget = null, }) {
    return (_jsxs(_Fragment, { children: [previewTargets.map((target) => (_jsx(HighlightBox, { target: target, showLabel: true }, `preview-${target.type}-${target.id}`))), markerPreviewTargets.map((target) => (_jsx(HighlightBox, { target: target, showLabel: true }, `marker-preview-${target.id}`))), mentionHighlightTarget ? (_jsx(HighlightBox, { target: mentionHighlightTarget, showLabel: true }, MENTION_HIGHLIGHT_KEY)) : null, showHoverInspect && hoveredTarget ? (_jsxs(_Fragment, { children: [_jsx(HighlightBox, { target: hoveredTarget }, HOVER_HIGHLIGHT_KEY), _jsx(InspectTooltip, { target: hoveredTarget })] })) : null, showSelectionHighlight && selectedTarget ? (_jsx(HighlightBox, { target: selectedTarget }, SELECTION_HIGHLIGHT_KEY)) : null, showPickProbeCompare && selectedTarget ? _jsx(PickTargetCompareChip, { target: selectedTarget }) : null, contextMenuTarget ? (_jsx(ContextMenuFocusBox, { target: contextMenuTarget }, CONTEXT_MENU_FOCUS_KEY)) : null, activeMarkerTarget ? (_jsxs(_Fragment, { children: [_jsx(HighlightBox, { target: activeMarkerTarget }, ACTIVE_MARKER_HIGHLIGHT_KEY), showActiveMarkerInspect ? _jsx(InspectTooltip, { target: activeMarkerTarget }) : null] })) : null] }));
}
