...
function MarkerTooltipScene() {
    ...
    const markers = useMemo(() => DEMO_MARKER_LAYOUT.map(({ left, top, reportIndex }, markerIndex) => {
    ...
    const [activeMarkerId, setActiveMarkerId] = useState(markers[0]?.id ?? null);
    ...
    return (_jsxs("div", { className: "relative h-full w-full p-[12px]", onMouseEnter: () => {
            enteredRef.current = true;
        }, onMouseLeave: handleStageLeave, children: [_jsxs("section", { className: "h-full overflow-hidden rounded-[18px] border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-surface)] shadow-[0_18px_44px_rgba(15,23,42,0.08)]", "aria-label": isKorean ? "마커가 배치된 데모 화면" : "Demo screen with feedback markers", children: [_jsxs("header", { className: "flex h-[54px] items-center gap-[10px] border-b border-[var(--adaptive-border-subtle)] px-[16px]", children: [_jsx("span", { className: "h-[28px] w-[28px] rounded-[9px] bg-[var(--adaptive-blue100)]" }), _jsxs("div", { className: "space-y-[5px]", children: [_jsx("span", { className: "block h-[7px] w-[94px] rounded-full bg-[var(--adaptive-black200)]" }), _jsx("span", { className: "block h-[6px] w-[58px] rounded-full bg-[var(--adaptive-black100)]" })] }), _jsx("span", { className: "ml-auto h-[26px] w-[76px] rounded-[8px] bg-[var(--adaptive-black100)]" })] }), _jsxs("div", { className: "grid grid-cols-[1.1fr_0.9fr] gap-[12px] p-[16px]", children: [_jsxs("article", { className: "rounded-[14px] border border-[var(--adaptive-border-subtle)] p-[14px]", children: [_jsx("span", { className: "block h-[8px] w-[54px] rounded-full bg-[var(--adaptive-blue200)]" }), _jsx("h3", { className: "mt-[12px] text-[17px] font-bold text-[var(--adaptive-black900)]", children: isKorean ? "팀 피드백을 한곳에서" : "Keep team feedback together" }), _jsx("p", { className: "mt-[7px] max-w-[220px] text-[11px] leading-[1.6] text-[var(--adaptive-black500)]", children: isKorean ? "화면의 정확한 위치에서 의견을 확인하고 해결하세요." : "Review and resolve feedback at the exact point it belongs." }), _jsx("div", { className: "mt-[16px] h-[34px] w-[124px] rounded-[10px] bg-[var(--adaptive-black900)]" })] }), _jsxs("div", { className: "grid gap-[10px]", children: [_jsxs("article", { className: "rounded-[14px] bg-[var(--adaptive-black100)] p-[12px]", children: [_jsx("span", { className: "block h-[7px] w-[58%] rounded-full bg-[var(--adaptive-black300)]" }), _jsx("span", { className: "mt-[8px] block h-[26px] w-[42%] rounded-[7px] bg-[var(--adaptive-surface)]" })] }), _jsxs("article", { className: "rounded-[14px] border border-[var(--adaptive-border-subtle)] p-[12px]", children: [_jsxs("div", { className: "flex items-center gap-[7px]", children: [_jsx("span", { className: "h-[24px] w-[24px] rounded-full bg-[var(--adaptive-green100)]" }), _jsx("span", { className: "h-[7px] flex-1 rounded-full bg-[var(--adaptive-black200)]" })] }), _jsx("span", { className: "mt-[10px] block h-[7px] w-[72%] rounded-full bg-[var(--adaptive-black100)]" })] })] })] })] }), markers.map((marker) => {
                const active = marker.id === activeMarkerId;
                return (_jsx(MarkerButton, { markerItem: marker, isHovered: active, isReportMode: false, isInteractive: true, isProximityHighlighted: false, isWindowOpen: false, viewingWindowBadge: messages.marker.viewingWindowBadge, detachedAriaLabel: messages.marker.detachedAriaLabel, detachedModalAriaLabel: messages.marker.detachedModalAriaLabel, markerAppearance: markerAppearance, typography: typography, onActivate: () => setActiveMarkerId((current) => (current === marker.id ? null : marker.id)), onHoverStart: () => setActiveMarkerId(marker.id), onHoverEnd: () => {
                        if (!introOpen) {
                            setActiveMarkerId((current) => (current === marker.id ? null : current));
                        }
                    }, onPointerMove: () => undefined, positioning: "absolute" }, marker.id));
            }), markers.map((marker, index) => marker.id === activeMarkerId ? (_jsx(MarkerTooltipSurface, { report: marker.report, detachedHint: messages.marker.detachedHint, detachedModalHint: messages.marker.detachedModalHint, positioning: "absolute", style: DEMO_MARKER_LAYOUT[index].tooltip }, marker.id)) : null)] }));
}
