...
function ElementInspectorScene() {
    const baseSession = useReportSession();
    const [values, setValues] = useState({ ...DEMO_PROBE_VALUES });
    const [open, setOpen] = useState(true);
    ...
    return (_jsx(ReportSessionContext.Provider, { value: session, children: _jsx("div", { className: "relative h-full w-full p-[12px]", children: _jsx(ProbeTooltip, { embedded: true }) }) }));
}
