...
export function FivePixelsDemo({ scene, locale, defaultLocale = "ko", onLocaleChange, showLocaleSwitch = false, appearance = "light", interactive = true, className = "", style, ariaLabel, }) {
    ...
    return (_jsxs(DemoRoot, { appearance: resolvedAppearance, width: size.width, height: size.height + localeSwitchHeight, interactive: interactive, className: className, style: style, ariaLabel: ariaLabel ?? `FivePixels ${scene} demo`, children: [showLocaleSwitch ? (_jsx("div", { className: "pointer-events-auto absolute right-0 top-0 z-[20] flex rounded-full border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-fillOpacity700)] p-[2px] shadow-sm backdrop-blur-[16px]", role: "group", "aria-label": copy.localeLabel, children: ["ko", "en"].map((option) => (_jsx("button", { type: "button", "aria-pressed": activeLocale === option, onClick: () => handleLocaleChange(option), className: `rounded-full px-[8px] py-[4px] text-[10px] ${activeLocale === option
                        ? "bg-[var(--adaptive-black900)] text-[var(--adaptive-surface)]"
                        : "text-[var(--adaptive-black500)]"}`, children: option.toUpperCase() }, option))) })) : null, _jsx("div", { className: "absolute inset-x-0 bottom-0", style: { height: size.height }, children: _jsx(DemoScene, { scene: scene, locale: activeLocale, copy: copy }) })] }));
}
