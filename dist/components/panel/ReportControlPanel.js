...
export function ReportControlPanel() {
    const { mode, roleStatItems, statusText, errorMessage, environment, projectId, appVersion, activeReplyReportId, showFeedbackList, showTargetPreview, isMobileViewport, panelTab, panelAppearance, setPanelAppearance, tooltipAppearance, setTooltipAppearance, questionThreadDisplay, setQuestionThreadDisplay, canTransferFeedback, personalKey, publicKey, personalKeyRequired, effectivePersonalKeyRequired, onboardingActive, keyGateActive, keyGateMode, messages, toggleReportMode, toggleTargetPreview, toggleIssueMode, openPanelTab, setErrorMessage, refetch, panelCollapsed, setPanelCollapsed, } = useReport();
    ...
    const feedbackBlocked = effectivePersonalKeyRequired || keyGateActive;
    ...
    const { panelRef, panelStyle, placementCorner, isDragging, activeCorner, handleDragHandlePointerDown } = usePanelDock({
        ...
        measureKey: `${panelCollapsed}-${isRecording}-${panelTab ?? "none"}-${isIssueMode}-${importStep !== "none" ? "import" : "none"}-${commandStep !== "none" ? "command" : "none"}-${keyGateActive ? "key-gate" : "ready"}`,
    });
    const panelExpanded = !panelCollapsed && !isRecording;
    const contentSectionOpen = panelTab !== null && personalKeyStep === "none" && importStep === "none" && !keyGateActive;
