...
export function PanelRoleSwitch() {
    ...
    return (_jsx(PanelDropdownMenu, { open: open, onClose: () => setOpen(false), trigger: _jsx(HoverTooltip, { label: messages.panel.roleSwitchAriaLabel, children: _jsxs("button", { type: "button", "aria-haspopup": "menu", "aria-expanded": open, "aria-label": messages.panel.roleSwitchAriaLabel, onPointerDown: (event) => {
                    event.stopPropagation();
                    setOpen((current) => !current);
                }, className: `flex h-[24px] shrink-0 items-center gap-[2px] rounded-[8px] px-[8px] text-[12px] font-semibold text-[var(--adaptive-black900)] ${open ? "bg-[var(--adaptive-black100)]" : "bg-[var(--adaptive-black300)]"}`, children: [_jsx("span", { className: "max-w-[72px] truncate", children: roleLabels[panelRole] }), _jsx(ChevronDownIcon, { className: `h-[14px] w-[14px] shrink-0 transition-transform ${open ? "rotate-180" : ""}` })] }) }), children: PANEL_ROLE_VALUES.map((role) => (_jsx(PanelDropdownMenuItem, { active: role === panelRole, onClick: () => handleSelect(role), children: roleLabels[role] }, role))) }));
}
