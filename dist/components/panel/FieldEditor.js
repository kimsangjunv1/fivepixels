...
function renderNonCheckboxField(field, { isDraftBubble, message, fieldValues, onMessageChange, onFieldChange, }) {
    if (field.key === "message") {
        return (_jsx("label", { className: `${isDraftBubble ? "flex flex-col gap-1" : "flex flex-col gap-1 text-xs"} w-full`, children: _jsx("textarea", { autoFocus: isDraftBubble, value: message, onChange: (event) => onMessageChange(event.target.value), className: "bg-[var(--adaptive-grey50)] p-[14px] focus:border-none focus:stroke-none focus:outline-none", placeholder: "\uD53C\uB4DC\uBC31\uC744 \uB0A8\uACA8\uC8FC\uC138\uC694" }) }, field.key));
    }
