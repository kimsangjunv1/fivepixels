...
function ThreadStartedDivider({ createdAt }) {
    ...
    return (_jsx(ThreadTimelineRow, { children: _jsxs("div", { className: "flex items-center gap-[8px]", role: "status", children: [_jsx("span", { "aria-hidden": true, className: "h-px flex-1 bg-[var(--adaptive-border-subtle)]" }), _jsxs("span", { className: "inline-flex shrink-0 items-center gap-[6px]", children: [_jsx("span", { "aria-hidden": true, className: "inline-flex h-[18px] w-[18px] items-center justify-center rounded-full", style: { backgroundColor: dateColor }, children: _jsx("span", { className: "h-[6px] w-[6px] rounded-full bg-white" }) }), _jsx("span", { className: "text-[13px] font-bold leading-none tabular-nums", style: { color: dateColor }, children: formatDateOnly(createdAt, locale) })] }), _jsx("span", { "aria-hidden": true, className: "h-px flex-1 bg-[var(--adaptive-border-subtle)]" })] }) }));
}
function CaseThreadEntry({ report, caseId, caseText, caseCreatedAt, caseStatus, actorName, pendingComposer, onStartAskQuestion, onClaimAssignee, isUpdating, isClaimingAssignee, isEditingCases = false, }) {
    ...
    const entryBody = (_jsxs(_Fragment, { children: [_jsx(FeedbackStatusBadge, { status: "issue_apply", isNeedGray: true }), _jsx("p", { className: `leading-[1.5] text-[14px] text-[var(--adaptive-text-primary)] whitespace-break-spaces ${caseStatus === "resolved" ? "text-[var(--adaptive-black500)] line-through" : ""}`, children: caseText }), report.author_name ? (_jsx(ThreadAuthorMeta, { authorName: report.author_name, createdAt: caseCreatedAt, showCreator: true })) : null, isEditingCases ? null : (_jsx(CaseThreadEntryActions, { report: report, caseId: caseId, actorName: actorName, pendingComposer: pendingComposer, onStartAskQuestion: onStartAskQuestion, onClaimAssignee: onClaimAssignee, isUpdating: isUpdating, isClaimingAssignee: isClaimingAssignee }))] }));
