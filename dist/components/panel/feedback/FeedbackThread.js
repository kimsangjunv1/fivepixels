...
function ThreadChildReply({ reply, cases, originalAuthorName, locale, threadReplyPrefix }) {
    return (_jsxs("article", { className: `flex flex-col gap-[4px] border-t border-[var(--adaptive-border-subtle)] ${threadReplyPrefix ? "py-[8px] pl-[18px]" : "py-[8px] pl-[12px]"}`, children: [_jsxs("div", { className: "flex items-start justify-between gap-[8px]", children: [_jsx(FeedbackStatusBadge, { status: reply.status }), _jsx("span", { className: "text-[12px] text-[var(--adaptive-black500)]", children: formatDate(reply.created_at, locale) })] }), _jsx(FeedbackCaseChips, { cases: cases, caseIds: reply.case_ids ?? [] }), _jsxs("p", { className: "leading-[1.5] text-[13px] text-[var(--adaptive-text-primary)]", children: [_jsx("span", { className: "text-[var(--adaptive-black400)]", children: threadReplyPrefix }), " ", reply.message] }), reply.author_name ? (_jsxs("div", { className: "flex items-center gap-[6px]", children: [_jsx("p", { className: "text-[12px] text-[var(--adaptive-black500)]", children: reply.author_name }), reply.author_name.trim() === originalAuthorName ? _jsx(FeedbackCreatorBadge, {}) : null] })) : null] }));
}
...
function ThreadIssueEntry({ report, children, locale, originalAuthorName, threadReplyPrefix, pendingComposer, onStartAskQuestion, onStartCheckout, isUpdating, }) {
    const { caseEditReportId, caseEditCases, beginCaseEdit, cancelCaseEdit, handleCaseEditSave, updateCaseEditDraftCase, addCaseEditDraftCase, removeCaseEditDraftCase, selectedCaseIds, toggleSelectedCase, errorMessage, } = useReport();
    ...
    return (_jsxs("div", { className: "flex flex-col", children: [_jsxs("article", { className: "flex flex-col gap-[4px] border-t border-[var(--adaptive-border-subtle)] p-[8px]", children: [_jsxs("div", { className: "flex items-start justify-between gap-[8px]", children: [_jsx(FeedbackStatusBadge, { status: "wait_for_reply" }), _jsx("span", { className: "text-[12px] text-[var(--adaptive-black500)]", children: formatDate(report.created_at, locale) })] }), _jsx(FeedbackCaseList, { cases: casesForEditor, isEditing: isEditingCases, canEdit: canEditReportCases(report) && !isEditingCases, isSaving: isUpdating, errorMessage: isEditingCases ? errorMessage : "", selectable: caseSelectionEnabled, selectedCaseIds: selectedCaseIds, onToggleCaseSelection: toggleSelectedCase, onBeginEdit: () => beginCaseEdit(report), onCancelEdit: cancelCaseEdit, onSaveEdit: () => void handleCaseEditSave(), onCaseChange: updateCaseEditDraftCase, onAddCase: addCaseEditDraftCase, onRemoveCase: removeCaseEditDraftCase }), report.author_name ? (_jsxs("div", { className: "flex items-center gap-[6px] px-[8px]", children: [_jsx("p", { className: "text-[12px] text-[var(--adaptive-black500)]", children: report.author_name }), _jsx(FeedbackCreatorBadge, {})] })) : null, !isEditingCases ? (_jsx(ThreadIssueEntryActions, { report: report, pendingComposer: pendingComposer, onStartAskQuestion: onStartAskQuestion, onStartCheckout: onStartCheckout, isUpdating: isUpdating })) : null] }), children.map((child) => (_jsx(ThreadChildReply, { reply: child, cases: report.cases, originalAuthorName: originalAuthorName, locale: locale, threadReplyPrefix: threadReplyPrefix }, child.id)))] }));
}
