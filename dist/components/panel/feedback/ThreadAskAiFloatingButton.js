...
export function ThreadAskAiFloatingButton({ report, fields, messages, caseId }) {
    ...
    return (_jsx(AskAiCopyDropdown, { align: "right", menuClassName: "min-w-[188px]", items: [
        ...
        ], trigger: ({ open, copied, toggle }) => (_jsx("button", { type: "button", "data-fivepixels-interactive": "", onPointerDown: (event) => event.stopPropagation(), onClick: toggle, "aria-expanded": open, "aria-haspopup": "menu", "aria-label": messages.marker.askAi.threadFloatingAriaLabel, title: copied ? messages.marker.askAi.copied : messages.marker.askAi.title, className: `flex h-[28px] w-[28px] items-center justify-center rounded-full border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black100)] text-[var(--adaptive-black700)] shadow-[var(--adaptive-popup-shadow)] transition-colors hover:bg-[var(--adaptive-black200)] hover:text-[var(--adaptive-black900)] ${open ? "text-[var(--adaptive-blue500)]" : ""}`, children: _jsx(AskAiIcon, { className: "h-[14px] w-[14px]" }) })) }));
}
