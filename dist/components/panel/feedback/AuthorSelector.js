...
export function AuthorSelector({ authors, value, onChange }) {
    ...
    return (_jsx(PanelDropdownMenu, { open: menuOpen, onClose: () => setMenuOpen(false), align: "left", menuClassName: "min-w-full", trigger: _jsxs("button", { type: "button", onClick: () => setMenuOpen((current) => !current), "aria-expanded": menuOpen, "aria-haspopup": "menu", "aria-label": "\uC791\uC131\uC790 \uC120\uD0DD", className: "flex w-full min-w-0 items-center justify-between rounded-full bg-[var(--adaptive-black800)] py-[4px] h-[24px] pr-[10px] pl-[12px] text-[12px] outline-none", children: [_jsx("span", { className: `text-[var(--adaptive-black500)]`, children: value || "작성자 선택" }), _jsx(ChevronDownIcon, { className: `h-[14px] w-[14px] shrink-0 text-[var(--adaptive-black600)] transition-transform ${menuOpen ? "rotate-180" : ""}` })] }), children: authors.map((author) => (_jsx(PanelDropdownMenuItem, { active: value === author.name, onClick: () => {
                setMenuOpen(false);
