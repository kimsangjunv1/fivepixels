...
export function FeedbackListItem({ report, locale, messages, listScope, disabled = false, canCreateGitHubIssue = false, creatingGitHubIssueId = null, onLocate, onDelete, onCreateGitHubIssue, }) {
    ...
    return (_jsx("div", { className: "group border-b border-[var(--adaptive-border-subtle)] last:border-b-0", onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), children: _jsxs("button", { type: "button", onClick: () => onLocate(report.id), className: "flex w-full flex-col gap-[8px] px-[16px] py-[8px] text-left", children: [_jsxs("section", { className: "flex flex-col gap-[4px]", children: [_jsxs("div", { className: "flex min-w-0 items-center justify-between gap-[4px]", children: [_jsxs("div", { className: "flex min-w-0 items-center gap-[4px]", children: [_jsx("span", { className: "truncate text-[14px] font-semibold text-[var(--adaptive-black900)]", children: caseId ?? "#FC-—" }), replyCount > 0 ? (_jsx("span", { className: "rounded-[4px] border-[2px] border-[var(--adaptive-black900)] bg-[var(--adaptive-black100)] px-[5px] py-[1px] text-[11px] font-semibold tabular-nums text-[var(--adaptive-black700)]", children: messages.feedbackList.replyCountBadge(replyCount) })) : null] }), _jsxs("span", { className: "flex shrink-0 items-center gap-[4px] text-[12px] tabular-nums text-[var(--adaptive-black500)]", children: [_jsx(ClockIcon, { className: "h-[12px] w-[12px]" }), formatTimeOnly(activityAt, locale)] })] }), _jsx("p", { className: "line-clamp-2 text-[14px] text-[var(--adaptive-black900)]", children: summary })] }), _jsxs("div", { className: "flex flex-wrap items-center gap-[6px]", children: [category ? (_jsxs("span", { className: "inline-flex items-center gap-[4px] rounded-[4px] bg-[var(--adaptive-black300)] px-[4px] py-[1px] text-[12px] font-medium text-[var(--adaptive-black700)]", children: [_jsx(CategoryShieldIcon, { className: "h-[11px] w-[11px] text-[var(--adaptive-black500)]" }), messages.feedbackList.categoryTag[category]] })) : null, _jsx("span", { className: "inline-flex items-center rounded-[4px] bg-[var(--adaptive-black300)] px-[4px] py-[1px] text-[12px] font-medium text-[var(--adaptive-black700)]", children: messages.feedbackList.statusTag[statusTag] })] }), listScope === "all" ? _jsx("p", { className: "truncate text-[11px] text-[var(--adaptive-black400)]", children: report.pathname }) : null] }) }));
}
