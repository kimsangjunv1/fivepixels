...
export function FeedbackListItem({ report, locale, messages, listScope, disabled = false, canCreateGitHubIssue = false, creatingGitHubIssueId = null, onLocate, onDelete, onCreateGitHubIssue, }) {
    ...
    return (_jsx("div", { className: "group border-b border-[var(--adaptive-border-subtle)] last:border-b-0", onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), children: _jsxs("button", { type: "button", onClick: () => onLocate(report.id), className: "flex w-full flex-col gap-[6px] px-[12px] py-[10px] text-left", children: [_jsxs("div", { className: "flex min-w-0 items-center justify-between gap-[8px]", children: [_jsxs("div", { className: "flex min-w-0 items-center gap-[6px]", children: [_jsx("span", { className: "truncate text-[13px] font-bold leading-[1.3] text-[var(--adaptive-black900)]", children: caseId ?? "#FC-—" }), replyCount > 0 ? (_jsx("span", { className: "rounded-[4px] border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black100)] px-[5px] py-[1px] text-[11px] font-semibold tabular-nums text-[var(--adaptive-black700)]", children: messages.feedbackList.replyCountBadge(replyCount) })) : null] }), hovered ? (_jsxs("div", { className: "flex shrink-0 items-center gap-[2px] rounded-full bg-[var(--adaptive-black900)] px-[6px] py-[2px]", onClick: (event) => event.stopPropagation(), children: [_jsx(FeedbackListCopyAction, { report: report, messages: messages }), canCreateGitHubIssue && onCreateGitHubIssue ? (_jsx(FeedbackListGitIssueAction, { report: report, messages: messages, disabled: disabled, isSubmitting: creatingGitHubIssueId === report.id, onCreateIssue: onCreateGitHubIssue })) : null, _jsx(FeedbackListDeleteAction, { report: report, onDelete: onDelete, disabled: disabled, messages: messages })] })) : (_jsxs("span", { className: "flex shrink-0 items-center gap-[4px] text-[12px] tabular-nums text-[var(--adaptive-black500)]", children: [_jsx(ClockIcon, { className: "h-[12px] w-[12px]" }), formatTimeOnly(activityAt, locale)] }))] }), _jsx("p", { className: "line-clamp-2 text-[12px] leading-[1.45] text-[var(--adaptive-black600)]", children: summary }), _jsxs("div", { className: "flex flex-wrap items-center gap-[6px]", children: [category ? (_jsxs("span", { className: "inline-flex items-center gap-[4px] rounded-[6px] bg-[var(--adaptive-black100)] px-[8px] py-[3px] text-[11px] font-medium text-[var(--adaptive-black700)]", children: [_jsx(CategoryShieldIcon, { className: "h-[11px] w-[11px] text-[var(--adaptive-black500)]" }), messages.feedbackList.categoryTag[category]] })) : null, _jsx("span", { className: "inline-flex items-center rounded-[6px] bg-[var(--adaptive-black100)] px-[8px] py-[3px] text-[11px] font-medium text-[var(--adaptive-black700)]", children: messages.feedbackList.statusTag[statusTag] })] }), listScope === "all" ? _jsx("p", { className: "truncate text-[11px] text-[var(--adaptive-black400)]", children: report.pathname }) : null] }) }));
}
