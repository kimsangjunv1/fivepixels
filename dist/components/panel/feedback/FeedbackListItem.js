...
export function FeedbackListItem({ report, locale, messages, listScope, disabled = false, canCreateGitHubIssue = false, creatingGitHubIssueId = null, onLocate, onDelete, onCreateGitHubIssue, }) {
    ...
    return (_jsx("div", { className: "group border-b border-[var(--adaptive-border-subtle)] last:border-b-0", onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), children: _jsxs("button", { type: "button", onClick: () => onLocate(report.id), className: "flex w-full flex-col gap-[6px] p-[10px_12px] text-left", children: [_jsx(FeedbackListRow, { text: _jsx("p", { className: "truncate text-[13px] leading-[1.4] text-[var(--adaptive-black900)]", children: getIssueSummary(report) }), trailing: hovered ? (_jsxs("div", { className: "flex items-center gap-[2px] rounded-full bg-[var(--adaptive-black900)] px-[6px] py-[2px]", onClick: (event) => event.stopPropagation(), children: [_jsx(FeedbackListCopyAction, { report: report, messages: messages }), canCreateGitHubIssue && onCreateGitHubIssue ? (_jsx(FeedbackListGitIssueAction, { report: report, messages: messages, disabled: disabled, isSubmitting: creatingGitHubIssueId === report.id, onCreateIssue: onCreateGitHubIssue })) : null, _jsx(FeedbackListDeleteAction, { report: report, onDelete: onDelete, disabled: disabled, messages: messages })] })) : (_jsx("span", { className: "rounded-full bg-[var(--adaptive-black900)] px-[8px] py-[2px] text-[12px] tabular-nums text-[var(--adaptive-black50)]", children: formatTimeOnly(activityAt, locale) })) }), _jsx("div", { className: "pl-[10px]", children: _jsx(FeedbackListRow, { text: latestReply ? (_jsxs("p", { className: "truncate text-[12px] leading-[1.4] text-[var(--adaptive-black600)]", children: [_jsx("span", { className: "text-[var(--adaptive-black400)]", children: messages.feedbackList.threadReplyPrefix }), " ", latestReply.message] })) : (_jsx("span", { className: "block h-[17px]" })), trailing: _jsxs("div", { className: "flex shrink-0 items-center gap-[6px]", children: [_jsx(FeedbackStatusBadge, { status: displayStatus }), remainingReplyCount > 0 ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-[11px] text-[var(--adaptive-black400)]", children: "|" }), _jsxs("span", { className: "text-[11px] tabular-nums text-[var(--adaptive-black500)]", children: ["+", remainingReplyCount] })] })) : null] }) }) }), listScope === "all" ? _jsx("p", { className: "truncate pl-[10px] text-[11px] text-[var(--adaptive-black400)]", children: report.pathname }) : null] }) }));
}
