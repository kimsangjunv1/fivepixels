...
export function ReportFeedbackList() {
    ...
    return (_jsxs("section", { className: "flex min-h-0 max-h-[51.2rem] flex-1 flex-col overflow-hidden rounded-[0_0_24px_24px] bg-[var(--adaptive-black50)]", children: [_jsxs("div", { className: "shrink-0 border-b border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)]", children: [filters.dateKey ? (_jsxs("div", { className: "flex items-center justify-between gap-[8px] border-b border-[var(--adaptive-border-subtle)] px-[8px] py-[6px]", children: [_jsx("p", { className: "text-[11px] font-[600] text-[var(--adaptive-blue500)]", children: messages.activityHeatmap.dateFilterLabel.replace("{date}", formatDateOnly(`${filters.dateKey}T12:00:00`, locale)) }), _jsx("button", { type: "button", onClick: () => setFilters((current) => ({ ...current, dateKey: null })), className: "text-[11px] font-[600] text-[var(--adaptive-black500)] hover:text-[var(--adaptive-black900)]", children: messages.activityHeatmap.clearDateFilter })] })) : null, _jsxs("div", { className: "flex items-center gap-[6px] px-[8px] py-[8px]", children: [canListAllFeedback ? (_jsx(PanelDropdownMenu, { open: scopeMenuOpen, onClose: () => setScopeMenuOpen(false), align: "left", trigger: _jsxs("button", { type: "button", onClick: () => setScopeMenuOpen((current) => !current), "aria-expanded": scopeMenuOpen, "aria-haspopup": "menu", "aria-label": messages.feedbackList.scopeAriaLabel, className: "flex h-[28px] min-w-[72px] items-center justify-between gap-[4px] rounded-[8px] border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)] px-[8px] text-[12px] text-[var(--adaptive-black800)] outline-none hover:bg-[var(--adaptive-black100)]", children: [_jsx("span", { className: "truncate", children: scopeLabel }), _jsx(ChevronDownIcon, { className: `h-[14px] w-[14px] shrink-0 text-[var(--adaptive-black600)] transition-transform ${scopeMenuOpen ? "rotate-180" : ""}` })] }), children: ["current", "all"].map((scope) => (_jsx(PanelDropdownMenuItem, { active: listScope === scope, onClick: () => {
                                        setScopeMenuOpen(false);
                                        setListScope(scope);
                                    }, children: scope === "current" ? messages.feedbackList.scopeCurrentPage : messages.feedbackList.scopeAllPages }, scope))) })) : null, _jsxs(PanelDropdownMenu, { open: statusMenuOpen, onClose: () => setStatusMenuOpen(false), align: "left", trigger: _jsxs("button", { type: "button", onClick: () => setStatusMenuOpen((current) => !current), "aria-expanded": statusMenuOpen, "aria-haspopup": "menu", "aria-label": messages.feedbackList.filterStatusAriaLabel, className: "flex h-[28px] min-w-[72px] items-center justify-between gap-[4px] rounded-[8px] border border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)] px-[8px] text-[12px] text-[var(--adaptive-black800)] outline-none hover:bg-[var(--adaptive-black100)]", children: [_jsx("span", { className: "truncate", children: statusLabel }), _jsx(ChevronDownIcon, { className: `h-[14px] w-[14px] shrink-0 text-[var(--adaptive-black600)] transition-transform ${statusMenuOpen ? "rotate-180" : ""}` })] }), children: [_jsx(PanelDropdownMenuItem, { active: filters.status === "all", onClick: () => {
