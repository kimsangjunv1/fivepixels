...
export function ReportPanelNoticeActionButton({ action, className = "", }) {
...
function NoticeFooter({ actions, footerDividerBeforeLast, className = "", }) {
    ...
    return (_jsx("div", { className: `flex items-center justify-end gap-[10px] ${className}`, children: actions.map((action, index) => (_jsxs("div", { className: "flex items-center gap-[10px]", children: [index === dividerIndex ? _jsx("div", { className: "h-[16px] w-[1px] bg-[var(--adaptive-black400)]" }) : null, _jsx(ReportPanelNoticeActionButton, { action: action })] }, action.id))) }));
}
/**
 ...
 */
export function ReportPanelNoticeDialog({ title, description, children, choices, actions, footerDividerBeforeLast = 0, sectioned = false, className = "", role = "dialog", }) {
