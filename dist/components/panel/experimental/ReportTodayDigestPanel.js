...
import { filterTodayDigest } from "../../../utils/experimentalPanelTabs.js";
...
export function ReportTodayDigestPanel() {
    ...
    const items = useMemo(() => filterTodayDigest(source), [source]);
    const handleOpen = (reportId) => {
        openPanelTab("feedback-list");
        void locateFeedback(reportId);
    };
    return (_jsxs("section", { className: "flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--adaptive-black50)]", children: [_jsxs("div", { className: "shrink-0 border-b border-[var(--adaptive-border-subtle)] px-[12px] py-[8px]", children: [_jsx("p", { className: "text-[12px] font-bold text-[var(--adaptive-black900)]", children: messages.panel.tabTodayDigest }), _jsx("p", { className: "mt-[2px] text-[11px] text-[var(--adaptive-black500)]", children: messages.panel.experimentalScopeAll })] }), _jsx("div", { className: "min-h-0 flex-1 overflow-y-auto", children: items.length === 0 ? (_jsx("p", { className: "px-[12px] py-[16px] text-[12px] text-[var(--adaptive-black500)]", children: messages.panel.experimentalEmpty })) : (items.map((report) => (_jsx(ExperimentalFeedbackRow, { report: report, onOpen: handleOpen }, report.id)))) })] }));
}
