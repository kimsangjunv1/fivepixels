...
function buildReviewerSnippet(info) {
    return `{ id: "${info.authorId}", name: "${info.name}", publicKey: "${info.publicKey}" }`;
}
function resolveOnboardingResumeState({ selfProfile, personalKey, publicKey, }) {
...
export function PanelOnboarding() {
    const { messages, panelRole, setPanelRole, completeOnboarding, restoreFromBackup, skipOnboarding, setErrorMessage, selfProfile, personalKey, publicKey, } = useReport();
    const onboarding = messages.onboarding;
    const resumeState = resolveOnboardingResumeState({ selfProfile, personalKey, publicKey });
    const [step, setStep] = useState(resumeState.step);
    const [name, setName] = useState(resumeState.name);
    const [isCreating, setIsCreating] = useState(false);
    const [completed, setCompleted] = useState(resumeState.completed);
