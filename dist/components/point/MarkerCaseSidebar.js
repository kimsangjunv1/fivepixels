...
export function MarkerCaseSidebar({ report, focusedCaseId, isComposingNewCase = false, composingCaseTitle, onSelectCase, onSelectComposingCase, }) {
    const { messages } = useReportPreferences();
    ...
    return (_jsxs("div", { className: "flex min-h-0 flex-1 flex-col overflow-hidden", children: [_jsx("p", { className: "shrink-0 px-[14px] pb-[8px] pt-[4px] text-[12px] font-semibold text-[var(--adaptive-black500)]", children: messages.cases.title }), _jsxs("ul", { className: "flex min-h-0 flex-1 flex-col gap-[2px] overflow-auto px-[6px] pb-[10px]", children: [cases.map((item) => {
                    ...
                    }), isComposingNewCase ? (_jsx("li", { children: _jsxs("button", { type: "button", "data-fivepixels-interactive": "", "aria-current": true, onClick: () => onSelectComposingCase?.(), className: "flex w-full flex-col items-start justify-center gap-[8px] rounded-[8px] bg-[var(--adaptive-neutralTintOpacity900)] px-[8px] py-[8px] text-left text-[var(--adaptive-black900)]", children: [_jsxs("section", { className: "flex w-full items-center gap-[4px]", children: [_jsx("span", { "aria-hidden": true, className: "inline-flex h-[12px] w-[12px] shrink-0 rounded-full border border-dashed border-[var(--adaptive-blue400)]" }), _jsx("span", { className: "min-w-0 flex-1 truncate text-[14px] font-semibold leading-[1] text-[var(--adaptive-blue400)]", children: resolvedComposingTitle })] }), _jsx("div", { className: "flex w-full min-w-0 items-center justify-between gap-[8px]", children: _jsx("span", { className: "shrink-0 whitespace-nowrap text-[11px] font-semibold leading-none text-[var(--adaptive-black500)]", children: messages.cases.open }) })] }) })) : null] })] }));
}
