...
function MarkerPinIcon({ pinned, className }) {
...
function UnfocusedCaseSummary({ caseTexts, emptyLabel }) {
    if (caseTexts.length === 0) {
        return _jsx("p", { className: "px-[24px] text-center text-[13px] text-[var(--adaptive-black500)]", children: emptyLabel });
    }
    return (_jsx("ul", { className: "flex max-h-full w-full max-w-[440px] flex-col gap-[8px] overflow-hidden px-[28px]", children: caseTexts.map((text, index) => (_jsxs("li", { className: "truncate text-center text-[13px] leading-[1.4] text-[var(--adaptive-black800)]", title: text, children: [_jsxs("span", { className: "mr-[6px] text-[var(--adaptive-black500)]", children: [index + 1, "."] }), text] }, `${index}-${text.slice(0, 24)}`))) }));
}
...
export function MarkerFeedbackWindow({ report, anchor, isFocused }) {
    const { messages, fields, authors, pendingComposer, replyDraft, replyMentions, replyAuthorName, confirmAuthorName, showConfirmAuthorSelect, errorMessage, setErrorMessage, isUpdating, isSubmittingReply, isClaimingAssignee, focusedCaseId, selectCase, closeReplyWindow, focusReplyWindow, minimizedReplyReportIds, setReplyWindowMinimized, clearHoverLeaveTimeout, scheduleHoverLeave, setHoveredMarkerId, setReplyDraft, setReplyMentions, setReplyAuthorName, setConfirmAuthorName, toggleConfirmAuthorSelect, handleReplySubmit, startDenyReview, startCheckoutReview, startAskQuestion, handleClaimAssignee, handleTransferAssignee, handleConfirmResolution, handleDelete, isDeleting, sessionActor, cancelPendingComposer, beginFeedbackEdit, addDraftCase, pinnedFeedbackItems, togglePinnedFeedback, } = useReport();
    ...
    const pinButton = (_jsx(WindowControlButton, { onClick: handleTogglePin, ariaLabel: isPinned ? messages.pins.unpinAriaLabel : messages.pins.pinAriaLabel, className: isPinned ? "text-[var(--adaptive-blue500)]" : "", children: _jsx(MarkerPinIcon, { pinned: isPinned, className: "h-[15px] w-[15px]" }) }));
    ...
    const expandedSidebarActions = (_jsxs("nav", { "aria-label": messages.marker.sidebarActionsAriaLabel, className: "shrink-0 px-[6px] pb-[10px] pt-[2px]", children: [canEditReportCases(report) ? (_jsxs("button", { type: "button", "data-fivepixels-interactive": "", onPointerDown: (event) => event.stopPropagation(), onClick: handleAddCase, className: SIDEBAR_ACTION_CLASS, children: [_jsx("span", { "aria-hidden": true, className: "inline-flex h-[15px] w-[15px] shrink-0 items-center justify-center text-[18px] font-normal", children: "+" }), _jsx("span", { children: messages.marker.newCaseAction })] })) : null, _jsx(MarkerWindowShareButton, { report: report, messages: messages, expanded: true }), _jsxs("button", { type: "button", "data-fivepixels-interactive": "", onPointerDown: (event) => event.stopPropagation(), onClick: handleTogglePin, "aria-pressed": isPinned, className: `${SIDEBAR_ACTION_CLASS} ${isPinned ? "text-[var(--adaptive-blue500)]" : ""}`, children: [_jsx(MarkerPinIcon, { pinned: isPinned, className: "h-[15px] w-[15px] shrink-0" }), _jsx("span", { children: isPinned ? messages.marker.unpinAction : messages.marker.pinAction })] })] }));
    ...
    const unfocusedBody = (_jsx("div", { className: "flex min-h-0 min-w-0 flex-1 items-center justify-center bg-[var(--adaptive-black50)]", children: _jsx(UnfocusedCaseSummary, { caseTexts: caseTexts, emptyLabel: messages.cases.selectToView }) }));
