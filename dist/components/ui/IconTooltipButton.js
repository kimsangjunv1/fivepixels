...
export function IconTooltipButton({ label, active = false, disabled = false, onClick, children, className = "" }) {
    return (_jsx(HoverTooltip, { label: label, disabled: disabled, children: _jsx("button", { type: "button", "aria-label": label, "aria-pressed": active, disabled: disabled, onPointerDown: (e) => {
                e.stopPropagation();
                onClick();
            }, className: `${ICON_BUTTON_BASE_CLASS} shrink-0 ${active ? "bg-[var(--adaptive-black100)]" : "bg-[var(--adaptive-black300)]"} ${className}`, children: children }) }));
}
