...
export function TargetHighlights({ hoveredTarget, previewTargets = [], selectedTarget }) {
    return (_jsxs(_Fragment, { children: [_jsxs(AnimatedPresence, { children: [previewTargets.map((target) => (_jsx(HighlightMotionBox, { target: target, showLabel: true }, `${target.type}-${target.id}`))), hoveredTarget ? (_jsx(HighlightMotionBox, { target: hoveredTarget, showLabel: true, layoutId: HOVER_LAYOUT_ID }, `hover-${hoveredTarget.type}-${hoveredTarget.id}`)) : null] }), selectedTarget ? (_jsx("div", { className: "pointer-events-none fixed box-border rounded-sm", style: {
