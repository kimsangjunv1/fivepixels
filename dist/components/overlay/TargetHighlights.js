...
function HighlightBox({ target, showLabel }) {
    return (_jsx("div", { className: highlight, style: {
        ...
        }, children: showLabel ? (_jsxs("span", { className: highlightLabel, style: { backgroundColor: TARGET_COLOR[target.type] }, children: [target.type, " \u00B7 ", target.id] })) : null }));
}
export function TargetHighlights({ hoveredTarget, previewTargets = [], selectedTarget }) {
    return (_jsxs(_Fragment, { children: [previewTargets.map((target) => (_jsx(HighlightBox, { target: target, showLabel: true }, `${target.type}-${target.id}`))), hoveredTarget ? _jsx(HighlightBox, { target: hoveredTarget, showLabel: true }) : null, selectedTarget ? (_jsx("div", { className: highlight, style: {
