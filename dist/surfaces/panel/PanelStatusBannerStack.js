...
import { ChevronLeftIcon, ChevronRightIcon } from "../../shared/components/icons/Icons.js";
...
import { PanelStatusBanner } from "./PanelStatusBanner.js";
...
export function PanelStatusBannerStack() {
    const { messages, showHiddenDetachedMarkers, setShowHiddenDetachedMarkers, showModalDetachedMarkers, setShowModalDetachedMarkers, } = useReportPreferences();
    const { markers, hasProbeSessionChanges, savedProbeCompareMode, setSavedProbeCompareMode, revertAllSavedProbeEdits, savedProbeEdits, canUndoProbeSession, canRedoProbeSession, undoProbeSessionAction, redoProbeSessionAction, } = useReportSession();
        ...
        if (hasProbeSessionChanges) {
            const editActions = [
                ...
                },
                {
                    ...
                    children: _jsx(ChevronLeftIcon, { className: "h-[14px] w-[14px]" }),
                },
                {
                    id: "edit-redo",
            ...
            next.push({
                ...
                actions: editActions,
                leading: _jsx(ProbeEditModeSpinner, {}),
                trailing: showCompare ? (_jsxs(_Fragment, { children: [_jsx(BannerDivider, {}), _jsx(PickTargetCompareSegment, { mode: savedProbeCompareMode, onChange: setSavedProbeCompareMode, beforeLabel: messages.pickTarget.probeBefore, afterLabel: messages.pickTarget.probeAfter, tone: "inverse" })] })) : null,
            });
        }
        if (hasHiddenMarker) {
            next.push({
    ...
    return (_jsx("div", { className: "flex shrink-0 flex-col", children: banners.map((banner, index) => (_jsx(PanelStatusBanner, { message: banner.message, actions: banner.actions, leading: banner.leading, trailing: banner.trailing, roundedTop: index === 0 }, banner.id))) }));
}
//# sourceMappingURL=PanelStatusBannerStack.js.map