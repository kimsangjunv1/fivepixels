...
function SettingsActionButton({ disabled = false, locked = false, lockLabel, onClick, children, }) {
    const button = (_jsx("button", { type: "button", disabled: disabled || locked, onClick: onClick, className: "w-full rounded-[8px] px-[12px] py-[8px] text-left text-[13px] text-[var(--adaptive-black800)] hover:bg-[var(--adaptive-black100)] disabled:cursor-not-allowed disabled:opacity-50", children: _jsxs("span", { className: "inline-flex items-center gap-[6px]", children: [locked ? _jsx(LockIcon, { className: "h-[12px] w-[12px] shrink-0 text-[var(--adaptive-black500)]" }) : null, children] }) }));
    if (!locked || !lockLabel) {
        return button;
    }
