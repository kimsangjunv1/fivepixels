...
import { formatHourLabel, resolveHourlyBarHeightPx } from "../../shared/utils/panel/hourlyCompareSparkline.js";
...
function SummarySide({ label, count }) {
    return (_jsxs("div", { className: "flex shrink-0 flex-col justify-center gap-[8px] p-[8px]", children: [_jsx("p", { className: "text-[14px] font-[600] leading-none text-[var(--adaptive-black500)]", children: label }), _jsx("p", { className: `text-[14px] font-bold leading-none text-[var(--adaptive-black900)] ${panelNumericClassName}`, children: count.toLocaleString() })] }));
}
function HourColumn({ bucket, maxCount, isHovered, isFuture, ariaLabel, onHover, onLeave, }) {
    ...
    return (_jsxs("button", { type: "button", "aria-label": ariaLabel, className: "group relative flex min-w-0 flex-1 flex-col items-center", onMouseEnter: (event) => onHover(bucket, event.clientX, event.clientY), onMouseMove: (event) => onHover(bucket, event.clientX, event.clientY), onMouseLeave: onLeave, onFocus: (event) => {
        ...
        }, onBlur: onLeave, children: [_jsx("span", { className: "flex-1 flex w-full items-end justify-center", style: { height: `${BAR_MAX_HEIGHT_PX}px` }, children: _jsx("span", { "aria-hidden": true, className: `w-full max-w-[8px] ${barClass}`, style: { height: `${todayHeight}px` } }) }), _jsx("span", { "aria-hidden": true, className: "h-[0.1px] w-full bg-[var(--adaptive-border-subtle)]" }), _jsx("span", { className: "flex-1 flex w-full items-start justify-center", style: { height: `${BAR_MAX_HEIGHT_PX}px` }, children: _jsx("span", { "aria-hidden": true, className: `w-full max-w-[8px] ${isHovered ? BAR_HOVER_CLASS : BAR_IDLE_CLASS}`, style: { height: `${yesterdayHeight}px` } }) })] }));
}
export function RouteDetailsTimeline({ sparkline, todayLabel, yesterdayLabel, timelineAriaLabel, hourAriaLabelTemplate, tooltipTodayTemplate, tooltipYesterdayTemplate }) {
    ...
    return (_jsxs("section", { className: "flex border-b border-b-[var(--adaptive-border-subtle)]", children: [_jsxs("div", { className: "flex shrink-0 flex-col justify-between border-r border-r-[var(--adaptive-border-subtle)]", children: [_jsx(SummarySide, { label: todayLabel, count: activeBucket?.todayCount ?? 0 }), _jsx("div", { className: "h-[0.1px] w-full bg-[var(--adaptive-border-subtle)]" }), _jsx(SummarySide, { label: yesterdayLabel, count: activeBucket?.yesterdayCount ?? 0 })] }), _jsx("div", { className: "flex min-w-0 flex-1 items-stretch gap-[1px]", role: "img", "aria-label": timelineAriaLabel, children: sparkline.buckets.map((bucket) => {
