...
import { AlignHorizontalCenterIcon, AlignHorizontalLeftIcon, AlignHorizontalRightIcon, AlignJustifyCenterIcon, AlignJustifyFlexEndIcon, AlignJustifyFlexStartIcon, AlignJustifySpaceBetweenIcon, AlignVerticalBottomIcon, AlignVerticalCenterIcon, AlignVerticalTopIcon, SwapHorizIcon, SwapVertIcon, ViewColumnIcon, ViewStreamIcon, } from "../../shared/components/icons/ProbeLayoutIcons.js";
import { buildFlexDirection, FLEX_ALIGN_VALUES, FLEX_JUSTIFY_VALUES, getFlexAxis, isColumnFlexDirection, isFlexReversed, stepGridTrackCount, stepProbeGap, toggleFlexReverse, } from "../../shared/utils/probe/probeLayout.js";
function ProbeLayoutSectionLabel({ children }) {
    return _jsx("span", { className: "text-[14px] font-medium leading-[1.45] text-[var(--adaptive-black500)]", children: children });
}
function ProbeIconToggleButton({ active, label, onClick, children, }) {
    return (_jsx("button", { type: "button", "data-fivepixels-interactive": "", "aria-label": label, title: label, onClick: onClick, className: `inline-flex h-[30px] w-[30px] items-center justify-center rounded-[8px] border border-solid transition-colors ${active
...
function ProbeCountStepperField({ label, value, onChange, }) {
    return (_jsxs("div", { className: "flex flex-col gap-[4px] text-[14px] leading-[1.45]", children: [_jsx(ProbeLayoutSectionLabel, { children: label }), _jsxs("div", { className: "flex items-center gap-[6px]", children: [_jsx("button", { type: "button", "data-fivepixels-interactive": "", onClick: () => onChange(stepGridTrackCount(value, -1)), className: "inline-flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[8px] border border-solid border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)] text-[14px] font-semibold text-[var(--adaptive-black900)]", "aria-label": `Decrease ${label}`, children: "\u2212" }), _jsx("div", { className: "min-w-0 flex-1 rounded-[8px] border border-solid border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)] px-[8px] py-[6px] text-center font-[var(--coding-font)] text-[14px] text-[var(--adaptive-black900)]", children: value }), _jsx("button", { type: "button", "data-fivepixels-interactive": "", onClick: () => onChange(stepGridTrackCount(value, 1)), className: "inline-flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[8px] border border-solid border-[var(--adaptive-border-subtle)] bg-[var(--adaptive-black50)] text-[14px] font-semibold text-[var(--adaptive-black900)]", "aria-label": `Increase ${label}`, children: "+" })] })] }));
}
