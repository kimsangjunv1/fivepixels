...
import { CloseIcon, AskActionIcon, DeniedActionIcon, CompleteActionIcon, SendIcon, KeyboardReturnIcon } from "../../shared/components/icons/Icons.js";
...
import { resolveComposerModeActionKind, THREAD_ACTION_STYLE } from "../../shared/constants/threadActionStyles.js";
import { stripMentionTokensForEmptyCheck } from "../../shared/utils/mention/elementMentions.js";
...
function ReplyTextarea({ value, onChange, placeholder, autoFocus, onSubmitShortcut, onMultilineChange, reserveInlineStart = 0, }) {
    ...
    return (_jsx("textarea", { ref: textareaRef, autoFocus: autoFocus, value: value, onChange: (event) => onChange(event.target.value), placeholder: placeholder, rows: 1, className: "max-h-[200px] w-full min-w-0 flex-1 resize-none overflow-hidden bg-transparent px-[4px] py-[6px] text-[14px] leading-[1.5] text-[var(--adaptive-black100)] outline-none placeholder:text-[var(--adaptive-text-muted)]", style: { minHeight: REPLY_TEXTAREA_MIN_HEIGHT }, onKeyDown: (event) => {
...
export function FeedbackComposer({ message = "", onMessageChange, mentions = [], onMentionsChange, userMentions = [], onUserMentionsChange, enableElementMentions = false, cases, onCaseChange, onAddCase, onRemoveCase, authorName, onAuthorNameChange, authors, fields: _fields, fieldValues: _fieldValues, onFieldChange: _onFieldChange, category = null, onCategoryChange, showCategory = false, showTags: _showTags = false, onSubmit, isSubmitting = false, showGitHubIssueOnCreate = false, onGitHubIssueSubmit, isGitHubIssueSubmitting = false, placeholder, autoFocus = false, errorMessage = "", showAskQuestionToggle = false, askQuestionChecked = false, onAskQuestionChange, askQuestionForced = false, composerMode = null, onCancelComposerMode, replyTargetPreview = null, hideAuthorSelector = false, lockedAuthorName, onFooterWarningChange, hideEditor = false, hideActions = false, hidePrimarySubmitAction = false, categoryPrompt, showCaseTabBar = true, activeCaseId, onActiveCaseIdChange, }) {
    const { messages, authors: teamAuthors, apiTeamMembers } = useReportPreferences();
    ...
    const emptyCaseIds = useMemo(() => (cases ?? [])
    ...
    return (_jsxs("div", { className: `flex w-full flex-col ${usesCaseEditor && !hideEditor ? "min-h-0 flex-1" : ""}`, children: [trimmedReplyTargetPreview ? (_jsx("div", { className: "px-[8px] pt-[8px]", title: trimmedReplyTargetPreview, children: _jsxs("div", { className: "flex min-w-0 items-center gap-[6px] rounded-[8px] bg-[var(--adaptive-black100)] px-[8px] py-[5px] text-[12px] leading-[1.4] text-[var(--adaptive-black500)]", children: [_jsx(KeyboardReturnIcon, { className: "h-[14px] w-[14px] shrink-0" }), _jsx("span", { className: "min-w-0 truncate", children: `"${trimmedReplyTargetPreview}"` })] }) })) : null, !hideEditor ? (_jsxs("div", { className: `relative ${usesCaseEditor ? "min-h-0 flex-1" : ""}`, children: [errorMessage && !isFooterHandledError ? (_jsx("div", { className: "absolute bottom-full left-0 right-0 z-10 mb-[6px]", children: _jsx(NoticeDialog, { role: "alert", title: messages.common.noticeTitle, description: errorMessage }) })) : null, usesCaseEditor ? (_jsx(FeedbackCaseEditor, { cases: cases, onCaseChange: onCaseChange, onAddCase: onAddCase, onRemoveCase: onRemoveCase, autoFocus: autoFocus, onSubmitShortcut: handleSubmit, needsAttention: caseNeedsAttention, attentionKey: caseAttentionKey, emptyCaseIds: emptyCaseIds, showTabBar: showCaseTabBar, activeCaseId: activeCaseId, onActiveCaseIdChange: onActiveCaseIdChange, enableElementMentions: enableElementMentions, placeholder: placeholder })) : enableElementMentions ? (_jsx("div", { "data-reply-measure-root": "", className: trimmedReplyTargetPreview ? "px-[8px] pt-[6px]" : "px-[8px] pt-[8px]", children: _jsxs("div", { className: showInlineComposerModeTag ? "flex items-start gap-[6px]" : undefined, children: [showInlineComposerModeTag && resolvedComposerMode ? (_jsx("div", { className: "flex h-[32px] shrink-0 items-center", children: _jsx(ComposerModeTag, { mode: resolvedComposerMode, messages: messages, onDismiss: onCancelComposerMode }) })) : null, _jsx(MentionComposerInput, { value: message, mentions: mentions, userMentions: userMentions, teamMembers: teamMembers, onChange: ({ message: nextMessage, mentions: nextMentions, userMentions: nextUserMentions }) => {
