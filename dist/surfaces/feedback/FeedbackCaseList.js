...
import { getCaseHandlerName } from "../../shared/utils/report/reportCases.js";
...
function AllCasesList({ report, cases, onSelectCase }) {
    ...
    return (_jsx("ul", { className: "flex flex-col gap-[6px] px-[8px] py-[4px]", children: cases.map((item, index) => {
            ...
            return (_jsx("li", { children: _jsxs("button", { type: "button", "data-fivepixels-interactive": "", onClick: () => onSelectCase(item.id), className: "flex w-full items-start gap-[6px] rounded-[8px] px-[4px] py-[6px] text-left hover:bg-[var(--adaptive-surface-muted)]/60", children: [_jsxs("span", { className: "w-[20px] shrink-0 tabular-nums text-[12px] text-[var(--adaptive-black500)]", children: [index + 1, "."] }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("span", { className: `text-[14px] leading-[1.5] text-[var(--adaptive-text-primary)] ${item.status === "resolved" ? "text-[var(--adaptive-black600)] line-through" : ""}`, children: mentionMessageToPlainText(item.text, item.mentions) }), isOpen ? (_jsxs("p", { className: "mt-[2px] text-[12px] text-[var(--adaptive-black500)]", children: [messages.cases.assignee, ": ", handlerName ?? messages.cases.unassigned] })) : null] }), _jsx(CaseResolvedBadge, { resolved: item.status === "resolved", resolvedLabel: messages.cases.resolved, openLabel: messages.cases.open })] }) }, item.id));
        }) }));
}
export function FeedbackCaseList({ report, cases, isEditing = false, canEdit = false, isSaving = false, errorMessage = "", focusedCaseId = null, onSelectCase, onAllTabActiveChange, onBeginEdit, onCancelEdit, onSaveEdit, onCaseChange, onAddCase, onRemoveCase, enableElementMentions = false, }) {
    const { messages } = useReportPreferences();
